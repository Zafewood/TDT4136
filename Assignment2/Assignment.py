...
class CSP:
    ...
    def backtrack(self, assignment):
        ...
        #if sentence to check that all var in assignment is not above 1
        if all(len(assignment.get(key)) == 1 for key in assignment.keys()):

        # var ← SELECT-UNASSIGNED-VARIABLE(csp, assignment)
          var = self.select_unassigned_variable(assignment)
        # for each value in ORDER-DOMAIN-VALUES(csp, var, assignment) do
          for value in assignment.get(var):
        # if value is consistent with assignment then
            if value in assignment.get(var):
    ...
    def inference(self, assignment, queue):
        ...
        # # queue ← a queue of arcs, initially all the arcs in csp (har den allerede)
