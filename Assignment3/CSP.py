...
class CSP:
    def __init__(self):
        ...
        #ADDED Count amount of backtracks and failures
        self.backtrackFailure = 0
        self.backtracks = 0
    ...
    def backtrack(self, assignment):
        ...
        """
        # TODO: YOUR CODE HERE
        #Up the counter on backtrack
        self.backtracks += 1
        
        # Gathers unassigned variables, which has more than 1 value in its domain
        unassigned_variable = self.select_unassigned_variable(assignment)

        # If unassigned_variable is None, all variables have been assigned a value and the assignment is returned
        if(unassigned_variable == None):
            return assignment
        
        # Loops through all unassigned keys in assignment
        for value in assignment[unassigned_variable]: 
            assignment_copy = copy.deepcopy(assignment) 
            assignment_copy[unassigned_variable] = [value]
            
            #If there is an inference, backtrack is called recursively
            if self.inference(assignment_copy, self.get_all_arcs()):
                result = self.backtrack(assignment_copy)
                if result is not None:
                    return result
                
        #Ups the counter on failure, when backtrack fails 
        self.backtrackFailure += 1
    ...
    def select_unassigned_variable(self, assignment):
        ...
        #Returns key that is assigned more than 1 value i.e. no assinged value!
    ...
    def inference(self, assignment, queue):
        ...
        #Loops through all arcs in queue to check for arc consistency using revise
        while queue:
            (Xi,Xj) = queue.pop()
            #Checks if revise is true, if so checks if assignment[Xi] is empty, if so returns false
            if self.revise(assignment, Xi, Xj):
                if len(assignment[Xi]) == 0:  
                    return False
                #If assignment[Xi] is not empty, checks all neighbors of Xi and adds them to queue to be inspected later.  
    ...
    def revise(self, assignment, i, j):
        ...
        #Adds legal constraints and sets revised to False (set to True when constraints are not satisfied)
        legal_constraints = self.constraints[i][j]
        revised = False
        for x in assignment[i]:
            # Sets Satisfied to False, set to True if value (x,y) in assignment[i]/[j]  is found in legal constraint
            satisfied = False 
            for y in assignment[j]:
                if (x, y) in legal_constraints: 
                    satisfied = True
                    break
            # If not satisfied, remove value from assignment[i] and set revised to True
            if not satisfied:
...
# Run code
def run(): 
    print("\nWelcome to Sudoku solver! Todo tasks solved by Steffen Rivedal Eimhjellen & Trygve Nummedal Os") 
    
    Task = int(input("Please enter task number (1-4) and press enter:"))
    ...
    match Task:
        ...
        case 3:
            txt_file = "hard.txt"
        case 4:
            txt_file = "veryhard.txt"
        case _:
            raise Exception("Invalid task number, please enter a number between 1-4")
    
    #Set up 
    csp = create_sudoku_csp(txt_file)
    solution = csp.backtracking_search()
    
    #Prints out the name of the txt file
    print("\nSudoku board:", txt_file + "\n")
    #Prints out the Sudoku board solution
    print_sudoku_solution(solution)

    #Prints out the amount of backtracks and failures
    print("\nAmount of backtracks:", csp.backtracks)
    print("Amount of failures:", csp.backtrackFailure)
